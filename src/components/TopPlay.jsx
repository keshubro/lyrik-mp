...
const TopChartCard = ({ song, i, isPlaying, activeSong, handlePauseClick, handlePlayClick }) => (
    <div className="w-full flex flex-row items-center hover:bg-[#4c426e] py-2 p-4 rounded-lg cursor-pointer mb-2">
        <h3 className="font-bold text-bse text-white mr-3">{i+1}.</h3>
...
const TopPlay = () => {
    ...
    const topPlays = data?.slice(0, 5);     // Take only first 5 from the array
    ...
    const _handlePauseClick = () => {
        dispatch(playPause(false));
    }

    const _handlePlayClick = (song, i) => {
        ...
        <div ref={divRef} className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col">
            <div className="w-full flex flex-col">
                ...
                <div className="mt-4 flex flex-col gap-1">
                    {topPlays?.map((song, i) => <TopChartCard key={song.key} song={song} i={i} isPlaying={isPlaying} activeSong={activeSong} handlePauseClick={() => _handlePauseClick()} handlePlayClick={() => _handlePlayClick(song, i)} />)}
                </div>
            </div>
